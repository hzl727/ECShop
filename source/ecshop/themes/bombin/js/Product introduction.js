...
aColor[i].onclick = function(){
	for(var j = 0; j <= aColor.length - 1; j ++){
		aColor[j].style.borderColor = '#d2d2d2';
...
$('#rtab li').each(function(i){
	$(this).click(function(){
		$('#rtab li').removeClass('now');		
		$(this).addClass('now');
		$('#rmain div[class="rmainbox"]').css('display','none');
		$($('#rmain div[class="rmainbox"]')[i]).css('display','block');
	})
});
